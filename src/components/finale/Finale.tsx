  ...
  useGSAP(
    () => {
      ...
      tl.fromTo(q(".fin-light"), { autoAlpha: 0.35, scale: 0.7 }, { autoAlpha: 1, scale: 1, duration: 0.5 }, 0)
        ...
        .to(q(".fin-ch"), { autoAlpha: 1, yPercent: 0, rotateX: 0, duration: 0.26, stagger: 0.012, ease: "power3.out" }, 0.06)
        .to(q(".fin-rise"), { autoAlpha: 1, y: 0, duration: 0.14, stagger: 0.03 }, 0.2);
    },
    ...
    <section ref={root} id="finale" data-scene aria-labelledby="finale-title" className={reducedMotion ? "scene relative" : finePointer ? "scene relative h-[210svh]" : "scene relative h-[150svh]"}>
      <div className="sticky top-0 h-[100svh] overflow-hidden bg-[radial-gradient(80%_70%_at_50%_60%,#4a0f1c_0%,#2a0810_45%,#120507_100%)]">
        ...
        <div className="relative z-10 flex h-full flex-col items-center justify-center px-6 pt-8 pb-[max(4.5rem,calc(3rem+env(safe-area-inset-bottom)))] text-center md:pt-0 md:pb-[26vh]">
          ...
          <h2 id="finale-title" className="mt-3 md:mt-5">
          ...
          <p className="fin-rise mt-4 font-display text-2xl italic text-champagne md:mt-6 md:text-4xl">{wedding.quotes.finale.en}</p>
          <p className="fin-rise tamil mt-4 max-w-3xl text-base text-gold-light/85 md:mt-7 md:text-2xl">{wedding.quotes.blessing.ta}</p>
          <p className="fin-rise mt-2 max-w-xl font-display text-base italic text-ivory/60 md:text-lg">{wedding.quotes.blessing.en}</p>
          <p className="fin-rise tamil mt-4 text-xl text-gold md:mt-6 md:text-2xl">{wedding.quotes.finale.ta}</p>
          ...
          <p className="fin-rise eyebrow mt-6 text-[0.7rem] tracking-[0.16em] text-ivory/70 md:mt-10 md:tracking-[0.34em]">
            {wedding.dates.range} · Palladam · Tirupur · {wedding.couple.hashtag}
          </p>
          <button
            type="button"
            onClick={() => scrollToTarget(0)}
            className="fin-rise mt-4 inline-flex min-h-11 items-center gap-2 rounded-full border border-gold/35 px-5 text-[0.7rem] uppercase tracking-[0.2em] text-gold-light/90 transition-colors md:mt-6 [@media(hover:hover)]:hover:text-gold-light"
          >
